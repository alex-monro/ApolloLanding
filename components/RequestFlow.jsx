...
/* What happens to a paste.
   ...
   size. Below xl they stack and the connectors turn to point down, minus the
        ...
        return (
          <li key={step.title} className="min-w-0 flex-1 xl:pr-8 xl:last:pr-0">
            <div className="flex items-center">
              ...
              {/* Across, at xl and up. The last step runs into a filled stop
              ...
              >
                <span className={`h-0 flex-1 border-t-2 ${line}`} />
                {isLast ? (
                  <span className="ml-4 flex size-10 shrink-0 items-center justify-center rounded-full border-2 border-solid border-ink bg-ink text-ground">
                    <Check className="size-[18px]" strokeWidth={2.5} />
                  </span>
                ) : (
                  <ArrowRight
                    className="-ml-px size-4 shrink-0 text-ink"
                    strokeWidth={2.5}
                  />
                )}
              </span>
            ...
            {/* Down, below xl. Nothing after the last step: a line running to a
